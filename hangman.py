...
def hangman(secretWord):
    ...
    * After each round, you should also display to the user the 
    ...
    while mistakesMade > 0 and not isWordGuessed(secretWord, letguessed):
        print('----------------')
        print('You have', mistakesMade, 'guesses left.')
        print('Available letters:', getAvailableLetters(letguessed))
        guess = input('Please guess a letter: ')
        if guess not in letguessed:
            letguessed.append(guess)

            if guess in secretWord:
                print('Good guess:', getGuessedWord(secretWord, letguessed))
            else:
                print("Oops! That letter is not in my word:", getGuessedWord(secretWord, letguessed))
                mistakesMade -= 1
        else:
